...
class Scramble():
    ...
    "3x3": (50, NOTATION), "2x2_L": (25, LEFT_HANDED), "2x2_R": (25, RIGHT_HANDED), "4x4": (95, (NOTATION, WIDE_NOTATION)), "Pyraminx": (25, PYRA_NOTATION), "Skewb": (25, SKEWB_NOTATION)
    }
...
class Timer():
    def __init__(self):
        ...
        self.currentMean = self.currentAverage = self.seconds = self.millieseconds = self.total = 0.0 #Defined as their specified data types wherever possible
        ...
        with open(filepath, "r") as file:
            ...
            for item in selectedItems:
                if item[0] != "DNF":
                    ...
                    self.total += seconds
                    self.filteredTimes.append(seconds)

            best_worst_times = self.best_and_worst_times()
            
            self.filteredTimes = [float(time) for time in self.filteredTimes if time not in best_worst_times] #appends times not included in best or worst time
            self.total = sum(self.filteredTimes)
            
            validity = len(self.filteredTimes)
    ...
    def best_and_worst_times(self):
        if self.validTimes:
            ...
            bestTime.config(text = "Best: " + minimum)
            worstTime.config(text = "Worst: " + maximum)
...
def timer_page():
    ...
    global scrambleText, timerText, puzzleChoice, timerWindow, handChoice, table, columns, numFails, numSolves, currentMean, bestMean, bestTime, worstTime, current_average_5, current_average_12, best_average_5, best_average_12
    ...
    foregroundButton = Button(buttonFrame, text = "Choose Foreground", command = lambda: colourChooser("foreground", [scrambleText, timerText, numSolves, numFails, currentMean, bestMean, bestTime, worstTime, current_average_5, current_average_12, best_average_5, best_average_12, originalButton, plus2Button, did_not_finish_button, generateScramble, ruleButton, refreshButton, submitButton, showFonts, foregroundButton, backgroundButton, saveButton, loadButton]))
    backgroundButton = Button(buttonFrame, text = "Choose Background", command = lambda: colourChooser("background", [timerWindow, timerFrame, buttonFrame, optionFrame, frame, tableFrame, statsFrame, scrambleText, timerText, numSolves, numFails, currentMean, bestMean, bestTime, worstTime, current_average_5, current_average_12, best_average_5, best_average_12, originalButton, plus2Button, did_not_finish_button, generateScramble, ruleButton, refreshButton, submitButton, showFonts, foregroundButton, backgroundButton, saveButton, loadButton]))
    ...
    buttonFrame.grid(row = 7, column = 0)
    showFonts.grid(row = 7, column = 1, padx = 5, pady = 5)
    ruleButton.grid(row = 7, column = 2, padx = 5, pady = 5)
    refreshButton.grid(row = 7, column = 3, padx = 5, pady = 5)
    foregroundButton.grid(row = 7, column = 4, padx = 5, pady = 5)
    backgroundButton.grid(row = 7, column = 5, padx = 5, pady = 5)
    saveButton.grid(row = 7, column = 6, padx = 5, pady = 5)
    loadButton.grid(row = 7, column = 7, padx = 5, pady = 5)
